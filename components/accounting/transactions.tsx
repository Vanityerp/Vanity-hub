...
export function Transactions({
  ...
  useEffect(() => {
    ...
    console.log('🔍 TRANSACTIONS COMPONENT: Filtered results:', {
      ...
      clientPortalTransactions: filtered.filter(tx => tx.source === 'CLIENT_PORTAL').length,
      filters
    });
    ...
    // Filter out duplicate transactions: keep only the latest per reference (appointment/booking/order)
const uniqueTransactionsMap = new Map();
sorted.forEach(tx => {
  // Use reference id if available, otherwise fallback to transaction id
  const refKey = tx.reference?.id || tx.id;
  // Always keep the latest (sorted is newest first)
  if (!uniqueTransactionsMap.has(refKey)) {
    uniqueTransactionsMap.set(refKey, tx);
  }
});
            ...
            {filteredTransactions.length === 0 ? (
              ...
              filteredTransactions.map((transaction) => (
                <TableRow key={transaction.id}>
                  ...
                  {/* Original Amount */}
                  <TableCell>
                    {/* Prefer explicit originalTotal, fallback to sum of service+product if available, else transaction.amount */}
                    {transaction.metadata?.originalTotal ? (
                      <CurrencyDisplay amount={transaction.metadata.originalTotal} />
                    ) : (transaction.originalAmount ? (
                      <CurrencyDisplay amount={transaction.originalAmount} />
                    ) : ((transaction.serviceAmount || transaction.productAmount) ? (
                      <CurrencyDisplay amount={(transaction.serviceAmount || 0) + (transaction.productAmount || 0)} />
                    ) : (
                      <CurrencyDisplay amount={transaction.amount} />
                    )))}
                  </TableCell>

                  {/* Final Amount (renamed from Amount) */}
                  <TableCell>
                    <div className="flex flex-col">
                      <CurrencyDisplay amount={transaction.amount} />
                      ...
                      {(transaction.discountPercentage && transaction.discountPercentage > 0) && (
                        <div className="text-xs text-green-600 mt-1">
                          {transaction.discountPercentage}% discount applied to services
                        </div>
                      )}
